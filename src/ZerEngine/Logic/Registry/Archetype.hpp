...
namespace zre::priv {
    ...
    public:
        ...
        template <typename... Comps>
        constexpr void directNewEnt(const Ent ent, Comps&&... comps) noexcept {
            size_t tmpLength = entIdx.size();
            if (!(tmpLength & maxCapacity)) {
                void* newChunk;
            ...
            if constexpr (sizeof...(Comps) > 0) {
                directNewEntRec(chunks.back(), tmpLength, std::forward<Comps>(comps)...);
            }
        }

        template <typename Comp, typename... Comps>
        constexpr void directNewEntRec(void* curChunk, const size_t tmpLength, Comp&& comp, Comps&&... comps) noexcept {
            Chunk::directSetComp(
                curChunk,
            ...
            if constexpr (sizeof...(Comps) > 0) {
                directNewEntRec(curChunk, tmpLength, std::forward<Comps>(comps)...);
            }
        }

        constexpr void newEnt(const Ent ent, const vector<LateUpgradeData>& comps) noexcept {
            size_t tmpLength = entIdx.size();
            if (!(tmpLength & maxCapacity)) {
        ...
        template <typename T>
        [[nodiscard]] constexpr T& getEnt(const Ent ent) noexcept {
        ...
        template <typename T>
        [[nodiscard]] constexpr const T& getEnt(const Ent ent) const noexcept {
            auto idx = entIdx.at(ent);
            return Chunk::get<T>(chunks[idx >> shiftMultiplier], idx & maxCapacity, rowSize, typeInfos.at(typeid(T).hash_code()).offset);
        }

        [[nodiscard]] constexpr void* getEntPtr(const Ent ent, const Type type) noexcept {
            auto idx = entIdx.at(ent);
            return Chunk::getPtr(chunks[idx >> shiftMultiplier], idx & maxCapacity, rowSize, typeInfos.at(type).offset);
        }

        inline void add(const Ent ent, Archetype& archOld, const vector<LateUpgradeData>& comps) noexcept {
            size_t tmpLength = entIdx.size();
        ...
        inline void del(const Ent ent, Archetype& archOld, const vector<LateUpgradeDelCompData>& comps) noexcept {
            size_t tmpLength = entIdx.size();
            if (!(tmpLength & maxCapacity)) {
                void* newChunk;
                if (rowSize << 2 <= pageSize)
