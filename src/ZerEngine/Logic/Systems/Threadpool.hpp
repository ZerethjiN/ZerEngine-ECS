...
namespace zre {
    ...
    class IFuncObj {
    public:
        virtual ~IFuncObj() noexcept = default;
        virtual void each() const noexcept = 0;
    };

    template <typename Func, typename... Comps>
    class FuncObj: public IFuncObj {
    public:
        constexpr FuncObj(const Func& newFunc, const Archetype* newArch):
        ...
        void each() const noexcept override final {
            for (const auto& pair: arch->entIdx) {
                if constexpr (std::is_invocable_v<Func, Comps&...>)
                    std::apply(func, std::make_tuple(std::ref(arch->getAt<Comps>(pair.second))...));
                else if constexpr (std::is_invocable_v<Func, Ent, Comps&...>)
                    std::apply(func, std::make_tuple(pair.first, std::ref(arch->getAt<Comps>(pair.second))...));
