...
namespace zre::priv {
    ...
    public:
        ...
        [[nodiscard]] inline const LiteQuery<Comps...> query(const With<Filters...>& = {}, const Without<Excludes...>& = {}) noexcept {
            ...
            for (auto* arch: archs) {
                if (arch->typeInfos.size() >= minlength) {
    ...
    private:
        ...
        [[nodiscard]] constexpr bool excludeCompatibleArchetype(const unordered_map<Type, TypeInfo>& sig) const noexcept {
    ...
    RenderThread::RenderThread(World& newWorld) noexcept:
        world(newWorld),
